...
Open ImageJ, update with `Help>Update ImageJ` to the latest version (currently, 1.54k required). Start ImageJ again and check the version.
...
Place the `ViPErLEED_ImageJ_plugins.jar` file (from this repository) into ImageJ/plugins or an immediate subdirectory thereof and restart ImageJ. The ViPErLEED plugins will appear in Plugins>ViPErLEED.

If you want to have the ViPErLEED plugins available with a single mouse click, add `run("LEED Commands");` to the "AutoRun" macro of your ImageJ StartupMacros file (in the ImageJ/plugins directory) or to the list of startup commands by selecting Edit>Options>Startup from the ImageJ Menu.
